...
class BF(nn.Module):
    def __init__(self, state_space, action_space, hidden_size, seed,
        ...
        # CNN snippet obtained from stable baselines 3
        self.cnn = nn.Sequential(
            nn.Conv2d(self.state_space.shape[-1],
                      16,
                      kernel_size=3,
                      stride=1,
                      padding=0), nn.ReLU(),
            nn.Conv2d(16, 32, kernel_size=2, stride=1, padding=0), nn.ReLU(),
            nn.Conv2d(32, 16, kernel_size=2, stride=1, padding=0), nn.ReLU(),
            nn.Flatten())

        # Compute shape by doing one forward pass
        with torch.no_grad():
            self.n_flatten = self.cnn(
                torch.FloatTensor(state_space.sample()).transpose(
                    0, 2).unsqueeze(0)).shape[1]

        self.linear = nn.Sequential(nn.Linear(self.n_flatten, action_space),
                                    nn.ReLU())

        # self.fc1 = nn.Linear(state_space, hidden_size)
        self.commands = nn.Linear(2, self.n_flatten)
        # self.fc2 = nn.Linear(hidden_size, hidden_size)
        # self.fc3 = nn.Linear(hidden_size, action_space)
        self.sigmoid = nn.Sigmoid()

    def forward(self, state, command):
        state = state if len(state.shape) == 4 else state.unsqueeze(0)
        out = self.cnn(state)
...
class UDRL():
    ...
    def create_training_input(self, episode, t1, t2):
        ...
        desired_reward = np.sum(episode["rewards"][t1:t2])
        time_horizont = t2 - t1
    ...
    def create_training_examples(self, batch_size):
        ...
        for ep in episodes:
            ...
            state, desired_reward, time_horizont, action = self.create_training_input(
                ep, t1, t2)
            states.append(state)
            desired_rewards.append(torch.FloatTensor([desired_reward]))
            desired_time_horizons.append(torch.FloatTensor([time_horizont]))
            actions.append(action)
        return actions, states, desired_rewards, desired_time_horizons
    ...
    def train_behavior_function(self, batch_size):
        ...
        """
        actions, states, desired_rewards, desired_time_horizons = self.create_training_examples(
            batch_size)

        states = torch.stack(states)
        d = torch.stack(desired_rewards)
        h = torch.stack(desired_time_horizons)
        command = torch.cat((d * self.return_scale, h * self.horizon_scale),
                            dim=-1)
        y = torch.stack(actions).long().view(-1)
        y_ = self.bf(states.to(self.device), command.to(self.device)).float()
    ...
    def evaluate(self, desired_return, desired_time_horizon):
        ...
        rewards = 0
        while True:
            state = torch.FloatTensor(state).transpose(0, 2)
        ...
        for i in range(self.n_warm_up_episodes):
            ...
            while True:
                action = self.bf.action(
                    torch.from_numpy(state).float().transpose(0, 2).to(
                        self.device), desired_return.to(self.device),
                    desired_time_horizon.to(self.device))
                next_state, reward, done, _ = self.env.step(action)
                states.append(torch.from_numpy(state).float().transpose(0, 2))
                actions.append(action)
                rewards.append(reward)
...
def main(trial):
    # init Environment
    wandb.init(entity="agkhalil", project="pytorch-udrl-procgen", reinit=True)
    wandb.watch_called = False
    config = wandb.config
    env = gym.make("procgen:procgen-coinrun-v0")
    action_space = env.action_space.n
    state_space = env.observation_space
    config.max_reward = 1200
