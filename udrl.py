...
class BF(nn.Module):
    ...
    def action(self, state, desire, horizon, novelty):
        """
...
class ReplayBuffer():
    ...
    def add_sample(self, states, next_states, actions, rewards,
                   intrinsic_rewards):
        episode = {
            "states": states,
            ...
            "summed_intrinsic_rewards": sum(intrinsic_rewards)
        }
    ...
    def get_random_samples(self, batch_size):
        self.sort()
        # TODO sort by intrinsic rewards
...
class UDRL():
    def __init__(self, env, action_space, state_space, max_reward,
                 ...
                 return_scale, horizon_scale, icm, init_desired_novelty):
        ...
        self.horizon_scale = horizon_scale
        self.icm = icm
        self.init_desired_novelty = init_desired_novelty
    ...
    def sampling_exploration(self):
        ...
        # TODO gather samples based on novelty not top performance
        ...
        intrinsic_returns = [i["summed_intrinsic_rewards"] for i in top_X]
        # from these returns calc the mean and std
        mean_returns = np.mean(returns)
        std_returns = np.std(returns)
        mean_intrinsic_returns = np.mean(intrinsic_returns)
        std_intrinsic_returns = np.std(intrinsic_returns)
        ...
        new_desired_intrinsic_reward = np.random.uniform(
            mean_intrinsic_returns,
            mean_intrinsic_returns + std_intrinsic_returns)

        return torch.FloatTensor([new_desired_reward]), torch.FloatTensor([
            new_desired_horizon
        ]), torch.FloatTensor([new_desired_intrinsic_reward])
    ...
    def create_training_input(self, episode, t1, t2):
        ...
        next_state = episode["next_states"][t1]
        desired_reward = sum(episode["rewards"][t1:t2])
        desired_novelty = sum(episode["intrinsic_rewards"][t1:t2])
        time_horizont = t2 - t1
        action = episode["actions"][t1]
        return state, next_state, desired_reward, time_horizont, action, desired_novelty
    ...
    def create_training_examples(self, batch_size):
        ...
        for ep in episodes:
            ...
            t2 = T
            state, next_state, desired_reward, time_horizont, action, desired_novelty = self.create_training_input(
                ep, t1, t2)
            input_array.append(
                torch.cat([
                    state, next_state,
                    ...
                    torch.FloatTensor([desired_novelty])
                ]))
    ...
    def train_behavior_function(self, batch_size):
        ...
        next_state = X[:, self.state_space:self.state_space * 2]
        d = X[:, self.state_space * 2:self.state_space * 2 + 1]
        h = X[:, self.state_space * 2 + 1:self.state_space * 2 + 2]
        n = X[:, self.state_space * 2 + 2:self.state_space * 2 + 3]
        ...
        latent_obs, latent_next_obs, next_obs_latent_hat, action_hat = self.icm.forward(
            state.to(self.device), next_state.to(self.device), y)
        forward_loss = F.mse_loss(latent_next_obs, next_obs_latent_hat)
        inverse_loss = F.mse_loss(y, action_hat.view(-1))
        curiosity_loss = forward_loss + inverse_loss

        curiosity_loss.backward()
        pred_loss.backward()
        self.optimizer.step()
        return pred_loss.detach().cpu().numpy(), curiosity_loss.detach().cpu(
        ).numpy()

    def evaluate(self, desired_return, desired_time_horizon, desired_novelty):
        """
        ...
        while True:
            ...
            action = self.bf.action(state.to(self.device),
                                    ...
                                    desired_novelty.to(self.device))
            next_state, reward, done, _ = self.env.step(action.cpu().numpy())
            _, next_state_latent, next_state_latent_hat, _ = self.icm.forward(
                state.to(self.device),
                torch.FloatTensor(next_state).to(self.device), action)
            intrinsic_reward = F.mse_loss(next_state_latent,
                                          next_state_latent_hat)
            rewards += reward
            ...
            desired_time_horizon = max(desired_time_horizon - 1,
                                       torch.FloatTensor([1]))
            desired_novelty = torch.FloatTensor([0])

            if done:
                break
        return rewards, intrinsic_rewards
    ...
    # Algorithm ICM - Generates an Episode using ICM
    def generate_episode_icm(self, desired_return, desired_time_horizon,
                             desired_novelty):
        """
        Generates more samples for the replay buffer.
        """
        ...
        while True:
            ...
            state = next_state
            desired_novelty -= intrinsic_reward

            if done:
                break
        return [states, next_states, actions, rewards, intrinsic_rewards]
    ...
    def run_upside_down(self, wandb, max_episodes):
        ...
        for ep in range(1, max_episodes + 1):
            ...
            for i in range(self.n_updates_per_iter):
                bf_loss, novelty_loss = self.train_behavior_function(
                    self.batch_size)
                loss_buffer.append(bf_loss)
                loss_novelty.append(novelty_loss)
            bf_loss = np.mean(loss_buffer)
            novelty_loss = np.mean(loss_novelty)

            # run x new episode and add to buffer
            for i in range(self.n_episodes_per_iter):
                # Sample exploratory commands based on buffer
                new_desired_reward, new_desired_horizon, new_desired_novelty = self.sampling_exploration(
                )
                generated_episode = self.generate_episode_icm(
                    new_desired_reward, new_desired_horizon,
                    new_desired_novelty)
                self.buffer.add_sample(*generated_episode)
            ...
            desired_novelty_history.append(new_desired_novelty.item())

            ep_rewards, ep_intrinsic_rewards = self.evaluate(
                new_desired_reward, new_desired_horizon, new_desired_novelty)

            wandb.log(
                {
                    "Episode reward": ep_rewards,
                    "Episode Intrinsic reward": ep_intrinsic_rewards,
                    "Desired horizon": new_desired_horizon.item(),
                    "Desired reward": new_desired_reward.item(),
                    "BF loss": bf_loss,
                    "Novelty loss": novelty_loss
                },
                step=ep)
    ...
    def warmup(self):
        ...
        for i in range(self.n_warm_up_episodes):
            ...
            desired_novelty = torch.FloatTensor([self.init_desired_novelty])
            state = self.env.reset()
            states = []
            next_states = []
            actions = []
            ...
            self.buffer.add_sample(states, next_states, actions, rewards,
                                   intrinsic_rewards)
...
def main(trial):
    ...
    wandb.init(entity="agkhalil",
               project="pytorch-udrl-novelty-cartpole",
               reinit=True)
    ...
    icm = ICM(env, config.device).to(config.device)
    optimizer = optim.Adam(params=chain(bf.parameters(), icm.parameters()),
                           lr=1e-3)
    udrl = UDRL(env, action_space, state_space, config.max_reward,
                ...
                bf, optimizer, config.return_scale, config.horizon_scale, icm,
                config.init_desired_novelty)
    udrl.warmup()
    udrl.run_upside_down(wandb, max_episodes=config.max_episodes)
