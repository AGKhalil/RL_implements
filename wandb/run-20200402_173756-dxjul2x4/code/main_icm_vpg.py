...
for episode in tqdm(range(0, EPISODES)):
    ...
    while not done:
        ...
        in_reward += next_obs_lat - next_obs_lat_hat
        ex_reward += rew
        ep_reward += rew + next_obs_lat - next_obs_lat_hat
        obs = new_obs

    rewards_size = len(rewards)
    rewards = np.add(np.subtract(next_obs_lat_hats, next_obs_lats), ext_rewards)
    ...
    loss_inv = loss_inv_fun(torch.Tensor(acts).squeeze(0), torch.stack(act_hats).squeeze(0))
    loss_for = loss_for_fun(torch.stack(next_obs_lats), torch.stack(next_obs_lat_hats))
